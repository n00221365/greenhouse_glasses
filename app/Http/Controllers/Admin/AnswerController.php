...
class AnswerController extends Controller
{
    ...
    public function index()
    {
        ...
        {
            return to_route('admin.answers.index');// if the person attempting to access is an admin, show them the admin index
        }

        $scores = Score::orderBy('created_at', 'desc')->paginate(4); // shows how many albums are displayed on each screen and in what order
...
#
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin.answers.index')->with('status', 'Created a new answer!');// brings the user back to the index upon completing the form
...

   {
    ...
    //dd($request);
       ...
       return redirect()->route('admin.answers.index')->with('status', 'Updated answer!');
   }
    /**
    ...
    public function destroy(string $id) // for deleting data from the database permenantly
    {
            ...
            return redirect()->route('admin.answers.index')->with('status', 'Selected answer deleted successfully!');
    }
}